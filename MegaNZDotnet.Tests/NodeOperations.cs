...
using MegaNZDotnet.Serialization;
...
public abstract class NodeOperations : TestsBase
{
    ...
    public void Move_Succeeds(NodeType destinationParentNodeType)
    {
        ...
        // Assert
        Assert.DoesNotContain(Context.Client.GetNodes(parentNode), x => x.Equals(node));
    ...
    [Theory]
    [JsonInputsData(new object[] { null }, new string[] { "FolderLink" })]
    [JsonInputsData(new object[] { "/file/SELECTED_FILE_NODE_ID" }, new string[] { "FolderLink" })]
    [JsonInputsData(new object[] { "/folder/SELECTED_FOLDER_NODE_ID" }, new string[] { "FolderLink" })]
